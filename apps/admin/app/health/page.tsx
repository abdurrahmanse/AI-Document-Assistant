  ...
  return (
    <div className="p-8 font-sans max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Infrastructure Health Dashboard</h1>
          <p className="text-sm text-gray-500 mt-2 font-mono">Environment: {data?.environment || "unknown"}</p>
        </div>
        <div className="flex items-center space-x-4">
          {isLoading && <span className="text-sm text-blue-500 font-medium animate-pulse">Syncing...</span>}
          <button
            onClick={toggleViewMode}
            className="px-4 py-2 bg-gray-100 text-gray-700 rounded-md shadow-sm hover:bg-gray-200 transition-colors"
          >
            {viewMode === "detailed" ? "Show Compact" : "Show Detailed"}
          </button>
          <button 
            onClick={() => refetch()}
            disabled={isLoading}
      ...
      {isError && (
        <div className="bg-red-50 border-l-4 border-red-500 p-4 mb-8">
          <h3 className="text-red-800 font-bold">API Connection Failed</h3>
        ...
        <div className="space-y-8">
          ...
          {viewMode === "detailed" && (
            <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg border border-gray-200 dark:border-gray-800">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Raw JSON Response</h3>
